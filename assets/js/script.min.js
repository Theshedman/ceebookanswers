const phoneInput = document.querySelector("#verify-subscription-phone"),
  ...
  verifiedPhone = document.querySelector("#verified-phone"), phoneTitle = document.querySelector("#phone-title"),
...
async function fetchApi () {
  ...
  phoneInput.value = "";
  const t = ` https://ceebookanswers.herokuapp.com/candidates/verifySubscription/${e}`;
  try {
...
const phoneField = document.querySelector("#admin-phone"), passwordField = document.querySelector("#admin-password"),
...
const submitLoginDetails = async (e, t) => {
    ...
    try {
      const e = await fetch(" https://ceebookanswers.herokuapp.com/admins/login", {
        method: "POST",
        mode: "cors",
...
guestLoginBtn.addEventListener("click", async e => {
  ...
  try {
    ...
    window.localStorage.setItem("token", o.data.token), window.location = "./guest-answers.html"
  } catch (e) {
...
const guestSubmitLoginDetails = async (e, t) => {
  const n = {phone: e, password: t};
  try {
    const e = await fetch(" https://ceebookanswers.herokuapp.com/guests/login", {
      method: "POST",
      mode: "cors",
      credentials: "same-origin",
      headers: {"Content-Type": "application/json"},
